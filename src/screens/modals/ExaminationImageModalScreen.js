...
export default class InsuranceImageModalScreen extends Component {
      ...
      <Container>
        <Content>
          <Grid style={{ paddingLeft: 5, paddingRight: 5, paddingTop: 50 }}>
              ...
              <Row size={20}>
                  <Col size={50}>
                      <Button full light onPress={() => this.addExaminationImage()}>
                          <Text>Kaydet</Text>
                      </Button>
                  </Col>
              </Row>
 ...
 addExaminationImage() {
    ...
    const carExaminationResponse = navigation.getParam('carExaminationResponse', {});
    const selectedCarId = navigation.getParam('selectedCarId', '-1');

    var request = new AddWehicleImageRequestModel();
    request.Token = token;
    request.ID = carExaminationResponse.ID;
    ...
    request.sigortaID = carExaminationResponse.sigortaID;
    request.plaka = carExaminationResponse.plaka;
    request.isDateRequired = "false";

    this.puantajService.addImage(request).then(responseJson => {        
        Alert.alert("Muayene resmi eklendi");
