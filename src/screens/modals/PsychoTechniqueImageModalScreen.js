...
export default class PsychoTechniqueImageModalScreen extends Component {
  utils = new Utils();
      ...
      <Container>
        <Content>
          <Grid style={{ paddingLeft: 5, paddingRight: 5, paddingTop: 50 }}>
              ...
              <Row size={20}>
                  <Col size={50}>
                      <Button full light onPress={() => this.addPsychoTechniqueImage()}>
                          <Text>Kaydet</Text>
                      </Button>
                  </Col>
              </Row>
  ...
  addPsychoTechniqueImage() {  
    ...
    const psychoTechniqueInsuranceInfo = navigation.getParam('psychoTechniqueInsuranceInfo', {});
    const personId = navigation.getParam('personId', '-1');

    var request = new AddWehicleImageRequestModel();
    request.Token = token;
    request.ID = psychoTechniqueInsuranceInfo.ID;
    request.startDate = this.state.chosenStartDate.toString();
    request.endDate = this.state.chosenEndDate.toString();
    request.entryID = personId;
    request.startEndDocumentType = "6"; //psikoteknik
    request.fileLocationType = "8"; //psikoteknik
    request.force = "false";
    request.image = this.state.selectedImageUri;
    request.sigortaID = psychoTechniqueInsuranceInfo.sigortaID;
    request.plaka = psychoTechniqueInsuranceInfo.plaka;
    ...
    this.puantajService.addImage(request).then(responseJson => {
        if (responseJson.IsSuccess) {
            Alert.alert("Psikoteknik resmi eklendi");
        }
        else {
