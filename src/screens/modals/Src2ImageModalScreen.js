...
export default class Src2ImageModalScreen extends Component {
  utils = new Utils();
      ...
      <Container>
        <Content>
          <Grid style={{ paddingLeft: 5, paddingRight: 5, paddingTop: 50 }}>
              ...
              <Row size={20}>
                  <Col size={50}>
                      <Button full light onPress={() => this.addSrc2Image()}>
                          <Text>Kaydet</Text>
                      </Button>
                  </Col>
              </Row>
  ...
  addSrc2Image() {  
    ...
    const src2InsuranceInfo = navigation.getParam('src2InsuranceInfo', {});
    const personId = navigation.getParam('personId', '-1');

    var request = new AddWehicleImageRequestModel();
    request.Token = token;
    request.ID = src2InsuranceInfo.ID;
    ...
    request.startEndDocumentType = "5"; //Src2
    request.fileLocationType = "7"; //Src2
    request.force = "false";
    request.image = this.state.selectedImageUri;
    request.sigortaID = src2InsuranceInfo.sigortaID;
    request.plaka = src2InsuranceInfo.plaka;
    ...
    this.puantajService.addImage(request).then(responseJson => {
        if (responseJson.IsSuccess) {
            Alert.alert("Src2 resmi eklendi");
        }
        else {
